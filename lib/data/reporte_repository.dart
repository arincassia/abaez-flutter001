...
class ReporteRepository {
  ...
  Future<void> crearReporte({
    ...
    try {
      ...
      throw ApiException('Error al procesar el reporte: ${e.toString()}', statusCode: 500);
    }
  }
  ...
  void _validarReporte(Reporte reporte) {
    ...
    if (reporte.fecha.isEmpty) {
      throw ApiException('La fecha del reporte es obligatoria', statusCode: 422);
    }
  ...
  Future<Reporte> obtenerReporte(String id) async {
    try {
      final Map<String, dynamic> reporteData = await _service.obtenerReporteRaw(id);
      return ReporteMapper.fromMap(reporteData);
    } on ApiException {
      rethrow;
    } catch (e) {
      throw ApiException('Error al procesar datos del reporte: ${e.toString()}', statusCode: 422);
    }
  }
  ...
  Future<List<Reporte>> obtenerReportes() async {
    try {
      final List<Map<String, dynamic>> reportesData = await _service.obtenerReportesRaw();
      
      return reportesData.map((data) => ReporteMapper.fromMap(data)).toList();
    } on ApiException {
      rethrow;
    } catch (e) {
      throw ApiException('Error al procesar lista de reportes: ${e.toString()}', statusCode: 422);
    }
  }
